...
class Moderation(commands.Cog):
	...
	async def kick(self, ctx, member : discord.Member = None, reason = None):
		...
		await ctx.channel.send(f"{member} has been kicked out by {ctx.author.name}", delete_after = 2.0)
	...
	async def ban(self, ctx, member : discord.Member = None, reason = None):
		...
		await ctx.send(f"{member} has been banned by **{ctx.author.name}**", delete_after = 2.0)
